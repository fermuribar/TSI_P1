...
public class AgenteDijkstra_inefiente extends AbstractPlayer {
	...
	private class Nodo implements Comparable<Nodo>{
		...
		public boolean equals(Object obj) {
			...
			return 	estado_general.getAvatarPosition().equals(o.estado_general.getAvatarPosition()) && 
					estado_general.getAvatarOrientation().equals(o.estado_general.getAvatarOrientation()) && 
					estado_general.getGameWinner() == o.estado_general.getGameWinner() ;
		}
	 ...
	 */
	public AgenteDijkstra_inefiente(StateObservation stateObs, ElapsedCpuTimer elapsedTimer){
	...
	private void Dijkstra(StateObservation stateObs){
		...
		while (!Solucion_encontrada && !abierotos.isEmpty()) {
			...
			if(nodo_actual.jugador_en_meta()){
				...
				Vector2d or_actual = nodo_actual.estado_general.getAvatarOrientation();
				...
				if(acciones_posibles.contains(Types.ACTIONS.ACTION_UP)){
					...
					if(orientacionToAction(or_actual) != Types.ACTIONS.ACTION_UP && hijo_arriba.estado_general.getAvailableActions().contains(Types.ACTIONS.ACTION_UP)){
						hijo_arriba.estado_general.advance(Types.ACTIONS.ACTION_UP);
						hijo_arriba.act_al_nodo.add(Types.ACTIONS.ACTION_UP);
						hijo_arriba.coste += 1;
					}
				...
				if(acciones_posibles.contains(Types.ACTIONS.ACTION_DOWN)){
					...
					if(orientacionToAction(or_actual) != Types.ACTIONS.ACTION_DOWN && hijo_abajo.estado_general.getAvailableActions().contains(Types.ACTIONS.ACTION_DOWN)){
						hijo_abajo.estado_general.advance(Types.ACTIONS.ACTION_DOWN);
						hijo_abajo.act_al_nodo.add(Types.ACTIONS.ACTION_DOWN);
						hijo_abajo.coste += 1;
					}
				...
				if(acciones_posibles.contains(Types.ACTIONS.ACTION_LEFT)){
					...
					if(orientacionToAction(or_actual) != Types.ACTIONS.ACTION_LEFT && hijo_izqu.estado_general.getAvailableActions().contains(Types.ACTIONS.ACTION_LEFT)){
						hijo_izqu.estado_general.advance(Types.ACTIONS.ACTION_LEFT);
						hijo_izqu.act_al_nodo.add(Types.ACTIONS.ACTION_LEFT);
						hijo_izqu.coste += 1;
					}
				...
				if(acciones_posibles.contains(Types.ACTIONS.ACTION_RIGHT)){
					...
					if(orientacionToAction(or_actual) != Types.ACTIONS.ACTION_RIGHT && hijo_dere.estado_general.getAvailableActions().contains(Types.ACTIONS.ACTION_RIGHT)){
						hijo_dere.estado_general.advance(Types.ACTIONS.ACTION_RIGHT);
						hijo_dere.act_al_nodo.add(Types.ACTIONS.ACTION_RIGHT);
						hijo_dere.coste += 1;
					}
	...
	private ACTIONS orientacionToAction(Vector2d or){
		if(or.x == 1)	return ACTIONS.ACTION_RIGHT;
		if(or.x == -1)	return ACTIONS.ACTION_LEFT;
		if(or.y == 1)	return ACTIONS.ACTION_DOWN;
		if(or.y == -1)	return ACTIONS.ACTION_UP;
